...
public class ForcesCommand extends AbstractAsyncPlayerCommand {
    ...
    protected CompletableFuture<Void> executeAsync(@Nonnull CommandContext ctx,
        ...
        PhysicsBodyId central = spawnBox(store, time, resource, space, centralPosition);
        ExamplePhysicsUtils.physicsOwnerRun(store, "apply central impulse demo",
            () -> ExamplePhysicsUtils.requireLiveBody(resource, central)
        ...
        PhysicsBodyId offCenter = spawnBox(store, time, resource, space, offCenterPosition);
        ExamplePhysicsUtils.physicsOwnerRun(store, "apply off-center impulse demo",
            () -> ExamplePhysicsUtils.requireLiveBody(resource, offCenter)
        ...
        PhysicsBodyId torque = spawnBox(store, time, resource, space, torquePosition);
        ExamplePhysicsUtils.physicsOwnerRun(store, "apply torque impulse demo",
            () -> ExamplePhysicsUtils.requireLiveBody(resource, torque)
        ...
        PhysicsBodyId force = spawnBox(store, time, resource, space, forcePosition);
        ExamplePhysicsUtils.physicsOwnerRun(store, "apply central force demo",
            () -> ExamplePhysicsUtils.requireLiveBody(resource, force)
