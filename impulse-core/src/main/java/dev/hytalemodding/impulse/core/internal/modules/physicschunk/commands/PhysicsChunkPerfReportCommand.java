...
public class PhysicsChunkPerfReportCommand extends AbstractAsyncWorldCommand {

    public PhysicsChunkPerfReportCommand() {
        super("report", "Report Impulse PhysicsChunk profiling metrics");
    }
    ...
    private static void sendReport(@Nonnull CommandContext ctx,
        ...
        if (cumulative.getTickSamples() <= 0) {
            ctx.sender().sendMessage(Message.raw("No profiled PhysicsChunk ticks recorded yet."
                + (profiling.enabled()
                ? ""
                : " Run /impulse physicschunk perf toggle, wait a few seconds, then run /impulse physicschunk perf report.")));
            return;
        ...
        ctx.sender().sendMessage(Message.raw("PhysicsChunk profiling: "
