...
public class PhysicsDebugSystem extends TickingSystem<ChunkStore> {
    ...
    public void tick(float dt, int index, @Nonnull Store<ChunkStore> store) {
        ...
        List<ImpulseBody> bodies = resource.getSpace().getBodies();

        for (ImpulseBody body : bodies) {
            ...
            switch (body.getShapeType()) {
                case BOX -> {
                    ...
                    DebugUtils.add(world, DebugShape.Cube, transform, DebugUtils.COLOR_LIME, 0.1f,
                ...
                case PLANE -> {
                    Matrix4d transform = new Matrix4d()
                        .translate(0, pos.y, 0)
                        .scale(20, 0.1, 20);
                    DebugUtils.add(world, DebugShape.Cube, transform, DebugUtils.COLOR_YELLOW, 0.1f,
                        DebugUtils.FLAG_NONE);
                }
                default -> {
                    Matrix4d transform = new Matrix4d()
                        ...
                        .rotate(rotD)
                        .scale(1);
                    DebugUtils.add(world, DebugShape.Cube, transform, DebugUtils.COLOR_RED, 0.1f,
                        DebugUtils.FLAG_NONE);
                }
            }
        }
